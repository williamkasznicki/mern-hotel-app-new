    ...
    <div className="mt-2 relative overflow-x-auto shadow-md sm:rounded-lg rounded-md dark:bg-gray-200">
      ...
      <PrintTableButton
        ...
        renderRow={(review: ReviewType) => `
          ...
          <td>${review.deleted_at ? 'Deleted' : 'Active'}</td>
        `}
        title="Review Data"
      />
      {/* Review Table */}
          ...
          <tbody>
            {reviews.map((review) => (
              ...
              >
                ...
                <td className="px-6 py-4">{review.hotelId}</td>
                <td className="px-6 py-4">{review.userName}</td>
                <td className="px-6 py-4">{review.starRating}</td>
                <td className="px-6 w-[400px] py-4 line-clamp-2">
                  {review.comment}
                </td>
      ...
      <div className="flex justify-between items-center p-4">
        ...
        <span className="font-semibold text-gray-900 dark:text-gray-900">
          {' '}
          (Total of {totalPages} pages and {reviews.length} records)
        </span>
