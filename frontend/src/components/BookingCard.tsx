...
const BookingCard = ({ booking }: { booking: BookingType }) => {
  ...
  const { data: roomNumber } = useQuery(
  ...
  if (!hotel || !roomNumber) {
    return <div>No hotel or room number found</div>;
  }
    ...
    <div className="border border-gray-300 bg-white shadow-lg rounded-lg overflow-hidden dark:bg-slate-800 dark:text-white">
      <Link to={`/detail/${hotel._id}`}>
        <div className="relative">
          <div
            className={`${
              isExpired() || booking.status === 'CANCELLED' ? 'grayscale' : ''
            }`}
          >
            <img
      ...
      <div className={`p-4 ${isExpired() ? 'grayscale' : ''}`}>
        ...
        <h3 className="text-sm font-semibold line-clamp-1">{hotel.address}</h3>
